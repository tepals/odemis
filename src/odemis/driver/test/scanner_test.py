...
class TestScanner(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for child in cls.sem_int.children.value:
            if child.name == CONFIG_SCANNER_INT["name"]:
                cls.ebeam_int = child
        cls.scanner = scanner.CompositedScanner(name="test", role="e-beam",
    ...
    def test_acquire_with_va(self):
        ...
        self.acq_done.wait(
            number * (2 + expected_duration * 1.1))  # 2s per image should be more than enough in any case
...
class TestDetector(unittest.TestCase):
    """
    Test detector with a CompositedScanner consisting of the SimSEM and SEMComedi as a child.
    """
...
class TestDetectorXTSEMComedi(TestDetector):
    """
    Test detector with a CompositedScanner consisting of the xt_client and SEMComedi as a child.
    """
